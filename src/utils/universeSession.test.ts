...
describe("universe session helpers", () => {
  it("derives display names and recent profiles from universe metadata or root path", () => {
    ...
    expect(universeDisplayName(vaultIndex("browser:Stories", { name: "Ever Archive" }))).toBe(
      "Ever Archive",
    );
    ...
    expect(
      profileForRecent(
        vaultIndex("C:/Vaults/Demo", { name: "Demo", icon: { type: "image", value: "icon.png" } }),
      ),
    ).toEqual({
      name: "Demo",
  ...
  it("deduplicates recent universes, keeps the newest first, and stores profiles by path", () => {
    ...
    const next = rememberUniverse(settings, "C", {
      name: "Gamma",
      icon: { type: "preset", value: "spark" },
    });

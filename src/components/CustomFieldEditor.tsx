...
import { Plus, Edit2, Trash2, GitBranch, List, DownloadCloud } from "lucide-react";
import type { PropertyDefinition, CustomFieldType } from "../editorTypes";
import { PropertyHierarchyEditor } from "./PropertyHierarchyEditor";
...
type CustomFieldEditorProps = {
  ...
  onChange: (fields: PropertyDefinition[]) => void;
};

type EditorViewMode = "flat" | "hierarchy";

type FieldItemProps = {
  field: PropertyDefinition;
  onUpdate: (field: PropertyDefinition) => void;
...
const FIELD_TYPE_LABELS: Record<CustomFieldType, string> = {
  ...
  file: "File",
  image: "Image",
  group: "Group (container for nested properties)",
};
    ...
    <div className="custom-field-item">
      {isEditing ? (
        <div className="custom-field-edit">
          {editingConditions ? (
            <PropertyConditionsEditor
              property={draft}
              allProperties={[field]}
              onSave={(visibleWhen) => {
                setDraft({ ...draft, visibleWhen });
            ...
            <>
              ...
              {draft.type !== "group" && (
                <label>
                  <span>Group/Category (optional):</span>
                  <input
                    type="text"
                    value={draft.group ?? ""}
                    onChange={(e) => setDraft({ ...draft, group: e.target.value })}
                    placeholder="e.g., abilities, editorial"
                  />
                </label>
              )}
...
export function CustomFieldEditor({ fields, onChange }: CustomFieldEditorProps) {
  ...
  const validation = validatePropertyStructure(fields);
  ...
  const handleAdd = () => {
    ...
    onChange([...fields, { ...newField, children: undefined }]);
    setNewField({
      id: "",
      label: "",
      type: "text",
  ...
  const handleLoadTemplate = (properties: PropertyDefinition[]) => {
    onChange(properties);
  ...
  return (
    <div className="custom-field-editor">
      ...
      <div className="custom-field-editor-tabs">
        <button
          className={`tab ${viewMode === "flat" ? "active" : ""}`}
      ...
      {viewMode === "flat" ? (
        ...
        <PropertyHierarchyEditor
          properties={fields}
          onChange={onChange}
          onSelectProperty={setSelectedProperty}
          selectedPropertyId={selectedProperty?.id}
          onAddChild={(parentId) => {
            // This would integrate with add child logic

...
export function themeForFamilyAndMode(family: ThemeFamily, mode: ThemeMode): ThemeId {
  return (
    THEMES.find((theme) => theme.family === family && theme.mode === mode)?.id ??
    "worldnotion-light"
  );
}
...
export function selectionColorForTheme(themeId: ThemeId): {
  ...
  const colorMap: Record<
    ThemeFamily,
    Record<ThemeMode, { backgroundColor: string; opacity: string }>
  > = {
    worldnotion: {
